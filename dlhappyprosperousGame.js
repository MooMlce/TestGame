...
var LZ4 = {
    ...
    init: (function() {
        ...
        LZ4.codec = (function() {
            var MiniLZ4 = (function() {
                ...
                exports.compressPackage = (function(data, verify) { // Разбитие файла на части
                    if (verify) {
                        var temp = new Uint8Array(exports.CHUNK_SIZE)
                    }
                    assert(data instanceof ArrayBuffer); // Фиксатор ошибки
    ...
    loadPackage: (function(pack) {
        ...
        if (!compressedData) compressedData = LZ4.codec.compressPackage(pack["data"]); // Пропускаем этот шаг т.к. нам пришел массив.
        assert(compressedData.cachedIndexes.length === compressedData.cachedChunks.length); // Фиксатор ошибки
        for (var i = 0; i < compressedData.cachedIndexes.length; i++) { // Функция записывающая чанки
            ...
            assert(compressedData.cachedChunks[i].length === LZ4.CHUNK_SIZE) // Фиксатор ошибки
        }
        pack["metadata"].files.forEach((function(file) {
    ...
    stream_ops: {
        read: (function(stream, buffer, offset, length, position) {
            ...
            while (written < length) {
                ...
                var compressedSize = compressedData.sizes[chunkIndex];//Остановка последняя. Размеры файлов sizes это массив разбивающий на части.
                var currChunk;
                if (compressedData.successes[chunkIndex]) {
                    ...
                    if (found >= 0) {
                        ...
                        var originalSize = LZ4.codec.uncompress(compressed, currChunk);// Записываем в массив с чанками наши файлы .mercury
...
var Browser = {
    ...
    init: (function() {
        ...
        imagePlugin["handle"] = function imagePlugin_handle(byteArray, name, onload, onerror) {
            ...
            var img = new Image;
            img.onload = function img_onload() { // Загрузка картинок на канвас
        ...
        try {
            ...
            try {
                if (webGLContextAttributes["majorVersion"] == 1 && webGLContextAttributes["minorVersion"] == 0) {
                    ctx = canvas.getContext("webgl", webGLContextAttributes) || canvas.getContext("experimental-webgl", webGLContextAttributes) // Место включения WebGl
...
Module["FS_createFolder"] = FS.createFolder; // ============== Пометка возможно тут начинается вся игра
